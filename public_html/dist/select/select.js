...
/* harmony import */ var bootbox__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! bootbox */ "./node_modules/bootbox/bootbox.all.js");
...
window.$=window.jQuery=$=jquery__WEBPACK_IMPORTED_MODULE_1___default.a;window.bootbox=bootbox__WEBPACK_IMPORTED_MODULE_2___default.a;window.alert=function(options){var message=options.message;var title=options.title?options.title:_t("warning");var okLabel=options.okLabel?options.okLabel:_t("ok");var cancelLabel=options.cancelLabel?options.cancelLabel:_t("cancel");var _callback=options.callback?options.callback:function(){};bootbox__WEBPACK_IMPORTED_MODULE_2___default.a.dialog({title:title,message:message,closeButton:false,buttons:{cancel:{label:cancelLabel,className:"btn-danger",callback:function callback(){}},ok:{label:okLabel,className:"btn-primary",callback:function callback(){return _callback();}}}});};/**
 * Ajax loader functions
 */$(document).on("submit",function(){$("body").append("<div class='loader'></div>");});$(document).ajaxSend(function(){$("body").append("<div class='loader'></div>");});$(document).ajaxComplete(function(){if($.active==1){$(".loader").remove();}});$(document).ajaxError(function(evt,request,settings){var data=request.responseText;if(data.length>0){try{var resp=JSON.parse(data);if(resp.messages[0]){bootbox__WEBPACK_IMPORTED_MODULE_2___default.a.alert(resp.messages[0].join("<br/>"));}}catch(ex){}}});$(document).ajaxSuccess(function(evt,request,settings){var data=request.responseText;if(data.length>0){try{var resp=JSON.parse(data);if(resp.messages[3]){bootbox__WEBPACK_IMPORTED_MODULE_2___default.a.alert(resp.messages[3].join("<br/>"));}}catch(ex){}}});var $=jquery__WEBPACK_IMPORTED_MODULE_1___default.a;
...
/* harmony import */ var bootstrap_select_v4__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! bootstrap-select-v4 */ "./node_modules/bootstrap-select-v4/dist/js/bootstrap-select.js");
/* harmony import */ var bootstrap_select_v4__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(bootstrap_select_v4__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _select_scss__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./select.scss */ "./base_theme/src/components/select/select.scss");
...
window.selectpicker=function(element){var func=arguments.length>1&&arguments[1]!==undefined?arguments[1]:_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"];Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(element).selectpicker(func);};Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(document).on("keyup",".bootstrap-select.autocomplete .bs-searchbox input",autocompleteFilter);var filter_caller=null;function autocompleteFilter(event){var input=String.fromCharCode(event.keyCode);var text=Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(this).val();//input key is not a character
if(text&&!/[a-zA-Z0-9-_ ]/.test(input)){return;}if(filter_caller){clearInterval(filter_caller);}var selectField=Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(this).parents(".bootstrap-select").find("select");filter_caller=setTimeout(function(){var data={token:Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(selectField).data("autocomplete-token"),data:text};_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"].ajax({url:root+"/ajax/autocompleteFilter",method:"post",data:data,success:function success(response){var response_data=JSON.parse(response);var options="";var selectedOptions=selectField.find("option:selected").map(function(i,option){return Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(option.outerHTML);});var nullOption=selectField.find("option[value='0']");options+=nullOption.length>0?null_option[0].outerHTML:"";var data=response_data.data;if(data instanceof Object){for(var id in data){options+="<option value='".concat(id,"'>").concat(data[id],"</option>");}}selectedOptions.each(function(i,selected){options+=selected[0].outerHTML;});selectField.html(options);if(selectField.hasClass("create-if-not-exist")){if(Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(selectField).find("option:contains('"+text+"')").length==0){Object(_jquery_jquery__WEBPACK_IMPORTED_MODULE_0__["$"])(selectField).append("<option value='"+text+"'>"+text+"</option>");}}selectField.selectpicker("refresh");}});},500);}
